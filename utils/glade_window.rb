...
class GladeWindow < DelegateClass(Gtk::Window)
	...
	def initialize(root_widget_name = nil, options = {})
		...
		instance_variable_names.each { |name| instance_variable_set('@' + name.to_s, @builder.get_object(name.to_s)) }

		# hookup signal handlers
		@builder.connect_signals { |handler_name| get_signal_handler(handler_name) }
